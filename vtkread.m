function [vertex,face] = vtkread(filename, verbose)
...
%the bastard just isn't reading anything lmao

[A,cnt] = fscanf(fid,'%f %f %f',3*nvert);

% writematrix(A,'A.txt','Delimiter',' ');
% type 'A.txt';

% writematrix(cnt,'cnt.txt','Delimiter',' ');
% type 'cnt.txt';
% 
% B=3*nvert;
% writematrix(B,'B.txt','Delimiter',' ');
% type 'B.txt';
