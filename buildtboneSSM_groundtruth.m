function buildtboneSSM_groundtruth()
...
surf_dir = fullfile(base_dir, 'ssm_surfaces_groundtruth');
h5_dir = fullfile(base_dir, 'ssm_H5_groundtruth');
...
template = 153;
[RT, LT] = findSegFileIndices(seg_dir, '.seg.nrrd', side, false, template, include_other_side);
...
for i=1:length(index)
    curr_data = adjustSegPath(seg_dir, '.seg.nrrd', side, index(i), false, false, false);
    mesh_path = adjustMeshPath(surf_dir, '.vtk', part, side, index(i), false, false, false, false);
    mesh_zero_path = adjustMeshPath(surf_dir, '.vtk', part, side, index(i), false, false, true, false);
...
if include_other_side
    for i=1:length(other_side_index)
        curr_data = adjustSegPath(seg_dir, '.seg.nrrd', other_side, other_side_index(i), false, false, false);
        mesh_path = adjustMeshPath(surf_dir, '.vtk', part, other_side, other_side_index(i), false, false, false, true);
        mesh_zero_path = adjustMeshPath(surf_dir, '.vtk', part, other_side, other_side_index(i), false, false, true, true);
...
for i=2:length(index)
    mesh_cpd_path = adjustMeshPath(cpd_surf_dir, '_cpd.vtk', part, side, index(i), false, false, false, false);
    mesh_path = adjustMeshPath(surf_dir, '.vtk', part, side, index(i), false, false, false, false);
...
if include_other_side
    for j=1:length(other_side_index)
        mesh_cpd_path = adjustMeshPath(cpd_surf_dir, '_cpd.vtk', part, other_side, other_side_index(j), false, false, false, true);
        mesh_path = adjustMeshPath(surf_dir, '.vtk', part, other_side, other_side_index(j), false, false, false, true);
